...
mod tests {
    ...
    fn resolve_provider_routes_ai_gateway_through_standard_openai_env() {
        assert_eq!(
            resolve_provider(
                ...
                None,
                None,
                Some("managed-by-gateway"),
                None,
            )
            .unwrap(),
            Provider::OpenAi
        );
        let err = resolve_provider(Some("ai-gateway"), None, Some("compat-token"), None, None)
            .unwrap_err();
    ...
    fn resolve_provider_openrouter_with_key() {
        assert_eq!(
            resolve_provider(Some("openrouter"), None, None, None, Some("sk-or-123")).unwrap(),
            Provider::OpenRouter
        );
    }
