...
mod provider;
use provider::OpenAiModelListResponse;
...
pub async fn get_agent_models(
    ...
    if let Some(models) = ai_gateway::discover_for_provider(
...
pub async fn discover_agent_models(
    ...
    if let Some(models) =
        ai_gateway::discover_for_provider(effective_provider.as_deref(), agent_command, None)
            .await?
    {
        return Ok(models);
    }
...
async fn discover_openai_compatible_models(
    ...
    if !relay_mesh && !provider::is_openai_compatible(provider.as_deref()) {
        return Ok(None);
    }
