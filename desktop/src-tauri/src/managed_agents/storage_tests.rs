...
impl KeyStore for FakeKeyStore {
    ...
    fn store_all_and_verify(&self, entries: &HashMap<String, String>) -> Result<(), String> {
        ...
        if entries
            .iter()
            .all(|(name, value)| self.stored.borrow().get(name) == Some(value))
        {
            Ok(())
        } else {
...
fn persist_agent_keys_batches_inline_keys_into_one_write() {
    // All inline keys are persisted in one keyring blob update. Besides being
    // atomic, this prevents macOS from prompting once per agent when access
    // control requires user confirmation.
    let store = FakeKeyStore::reachable();
    let mut records = vec![
