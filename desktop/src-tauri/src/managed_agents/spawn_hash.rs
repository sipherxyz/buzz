...
pub(crate) fn spawn_config_hash(
    ...
    // Resolve prompt/model/provider once, using the same source of truth as
    // `spawn_agent_child`. The provider also decides whether the agent command
    // is wrapped by AI Gateway, so it must be available before hashing the
    // launch derivations below.
    ...
    runtime_meta
        ...
        .unwrap_or("")
        .hash(&mut hasher);
    if super::is_ai_gateway_provider(resolved_provider.as_deref()) {
        "ai-gateway".hash(&mut hasher);
        super::selected_profile_for_env(&descriptor.env)
